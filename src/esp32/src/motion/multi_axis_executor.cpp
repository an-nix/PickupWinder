...
void MultiAxisExecutor::run()
{
    ...
    auto requestPlannerFlush = [&](uint16_t motion_sequence) {
        flush_request_t req {
            .flush_sequence = motion_sequence,
            .source = FLUSH_SOURCE_INTERNAL,
            .reserved = 0,
        };
    ...
    for (;;) {
        ...
        case ExecState::RECOVERY: {
            // Drain stale host flushes that were queued before the endstop hit,
            // but preserve the newest internal flush request generated by the
            // executor itself so the planner still receives the recovery flush.
            {
                flush_request_t queued_flush;
                flush_request_t latest_internal_flush {};
                bool has_internal_flush = false;
                while (xQueueReceive(runtime_.flushQueue(), &queued_flush, 0) == pdTRUE) {
                    if (queued_flush.source == FLUSH_SOURCE_INTERNAL) {
                        if (!has_internal_flush ||
                            sequence_is_newer_u16(queued_flush.flush_sequence,
                                                 latest_internal_flush.flush_sequence)) {
                            latest_internal_flush = queued_flush;
                            has_internal_flush = true;
                        }
                        continue;
                    }
                    ESP_LOGW(TAG, "recovery: discarding stale host flush seq=%u",
                             static_cast<unsigned>(queued_flush.flush_sequence));
                }
                if (has_internal_flush
                    && xQueueSend(runtime_.flushQueue(), &latest_internal_flush, 0) != pdTRUE) {
                    ESP_LOGW(TAG, "recovery: failed to restore internal flush seq=%u",
                             static_cast<unsigned>(latest_internal_flush.flush_sequence));
                }
            }

            planned_segment_t discard;
            uint32_t drained = 0;
            uint16_t last_drained_seq = 0xFFFFu;
