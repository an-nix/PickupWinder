...
static size_t rmt_encode_stepper_motor_curve(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    if (motor_encoder->flags.is_accel_curve) {
        encoded_symbols = copy_encoder->encode(copy_encoder, channel, &motor_encoder->curve_table[0], points_num * sizeof(rmt_symbol_word_t), &session_state);
    } else {
        encoded_symbols = copy_encoder->encode(copy_encoder, channel, &motor_encoder->curve_table[0] + motor_encoder->sample_points - points_num, points_num * sizeof(rmt_symbol_word_t), &session_state);
    }
...
// ========================= Batch Uniform Encoder (generates N symbols) =========================
typedef struct {
    rmt_encoder_t base;
    rmt_encoder_handle_t copy_encoder;
    uint32_t symbol_count;
    rmt_symbol_word_t symbol_table[];
} rmt_stepper_uniform_batch_encoder_t;

RMT_ENCODER_FUNC_ATTR
static size_t rmt_encode_stepper_motor_uniform_batch(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    rmt_stepper_uniform_batch_encoder_t *motor_encoder = __containerof(encoder, rmt_stepper_uniform_batch_encoder_t, base);
    rmt_encoder_handle_t copy_encoder = motor_encoder->copy_encoder;
    rmt_encode_state_t session_state = RMT_ENCODING_RESET;
    
    // Encode all symbols in the pre-built table (ignore primary_data since symbols are pre-built)
    size_t encoded_symbols = copy_encoder->encode(copy_encoder, channel, &motor_encoder->symbol_table[0],
                                                   motor_encoder->symbol_count * sizeof(rmt_symbol_word_t), &session_state);
    *ret_state = session_state;
    return encoded_symbols;
}

static esp_err_t rmt_del_stepper_motor_uniform_batch_encoder(rmt_encoder_t *encoder)
{
    rmt_stepper_uniform_batch_encoder_t *motor_encoder = __containerof(encoder, rmt_stepper_uniform_batch_encoder_t, base);
    rmt_del_encoder(motor_encoder->copy_encoder);
    free(motor_encoder);
...
RMT_ENCODER_FUNC_ATTR
static esp_err_t rmt_reset_stepper_motor_uniform_batch_encoder(rmt_encoder_t *encoder)
{
    rmt_stepper_uniform_batch_encoder_t *motor_encoder = __containerof(encoder, rmt_stepper_uniform_batch_encoder_t, base);
    rmt_encoder_reset(motor_encoder->copy_encoder);
    return ESP_OK;
...
{
    esp_err_t ret = ESP_OK;
    rmt_stepper_uniform_batch_encoder_t *step_encoder = NULL;
    ESP_GOTO_ON_FALSE(config && ret_encoder, ESP_ERR_INVALID_ARG, err, TAG, "invalid arguments");
    ESP_GOTO_ON_FALSE(config->symbol_count > 0, ESP_ERR_INVALID_ARG, err, TAG, "symbol count must be > 0");
    
    step_encoder = rmt_alloc_encoder_mem(sizeof(rmt_stepper_uniform_batch_encoder_t) + config->symbol_count * sizeof(rmt_symbol_word_t));
    ESP_GOTO_ON_FALSE(step_encoder, ESP_ERR_NO_MEM, err, TAG, "no mem for stepper batch uniform encoder");
    rmt_copy_encoder_config_t copy_encoder_config = {};
    ESP_GOTO_ON_ERROR(rmt_new_copy_encoder(&copy_encoder_config, &step_encoder->copy_encoder), err, TAG, "create copy encoder failed");

    // Pre-compute all symbols with the same frequency
    uint32_t symbol_duration = config->resolution / config->target_freq_hz / 2;
    for (uint32_t i = 0; i < config->symbol_count; i++) {
        step_encoder->symbol_table[i].level0 = 0;
        step_encoder->symbol_table[i].duration0 = symbol_duration;
        step_encoder->symbol_table[i].level1 = 1;
        step_encoder->symbol_table[i].duration1 = symbol_duration;
    }
    
    step_encoder->symbol_count = config->symbol_count;
    step_encoder->base.del = rmt_del_stepper_motor_uniform_batch_encoder;
    step_encoder->base.encode = rmt_encode_stepper_motor_uniform_batch;
    step_encoder->base.reset = rmt_reset_stepper_motor_uniform_batch_encoder;
