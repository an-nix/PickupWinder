# Architecture V6 — PickupWinder Host

## 1) Changelog V5 → V6

| Fichier | Correction appliquée | Décision technique retenue |
|---|---|---|
| src/rpi/transport/streamer.py | `from_axis_ids()` accepte désormais `segment_duration_s` | Évite le verrouillage implicite à `0.004s` pour les chemins `WoundMove`; le constructeur alternatif devient cohérent avec les générateurs externes. |
| src/rpi/transport/streamer.py | Suppression de la duplication des calculs de buffer-time | Introduction d’un helper unique `_safe_buffer_time_s(requested_time_s, max_hz)` utilisé par les deux chemins (`RampConfig` et `target_hz` explicite). |
| src/rpi/motion/move_queue.py | `_make_wound_streamer()` transmet `move.segment_duration_s` | Aligne la granularité temporelle de streaming avec celle réellement demandée par le move synchronisé. |
| src/rpi/motion/move_queue.py | `_execute_wound_move()` invalide la position aussi sur branche stop | La sémantique d’un `WoundMove` reste non-prédictive sur les 3 sorties: endstop, stop, succès. |
| src/rpi/motion/engine.py | Ajout d’un logger module + warning de cohérence profil/géométrie | `wound_run()` nève pas d’exception; warning si écart relatif > 5% entre tours géométriques attendus et tours produits par `(accel, cruise, decel)`. |
| src/rpi/jsonrpc/handlers.py | Implémentation de `AppRpcHandler.config()` | Évite l’échec runtime sur `winder.config`; sérialisation robuste des dataclasses et objets de config. |
| tests/test_motion_v3.py | Régressions V6 ajoutées | Couvre: invalidation sur stop, warning de cohérence `wound_run`, propagation de `segment_duration_s` dans `from_axis_ids()`. |

---

## 2) Sources complètes — fichiers modifiés en V6
...
## 3) Sources complètes — fichiers demandés en analyse (non modifiés)
...
## 4) Points d’attention restants (post-V6)

1. `AppRpcHandler` dans `winding_main.py` est construit sans `app`, donc `winder.config` retourne actuellement `{}` en production tant que l’application n’est pas injectée.
2. `motion/syncrhonized_segment_generator.py` garde une faute de nom de fichier (`syncrhonized`), déjà propagée dans les imports. Correction possible mais breaking pour les imports existants.
3. `streamer.py` contient des imports non utilisés (`Iterator`, `List`) à nettoyer si vous imposez un lint strict.
4. Les méthodes privées `streamer._generator` et `streamer._generator_finished` sont mutées depuis `MoveQueue`; c’est un couplage interne fort (dette design) mais inchangé.
5. Le warning de cohérence `wound_run` n’est pas émis si `expected_turns <= 0` (comportement volontaire); à surveiller si des appels “géométrie nulle” doivent être explicitement refusés.

---

## 5) Diagramme Mermaid V6 (flux dépendances & points critiques)

```mermaid
flowchart TD
    A[JSON-RPC: winding.*] --> B[WindingEngine]
    B --> C[MoveQueue]
    C --> D[MultiAxisRampStreamer]
    D --> E[Esp32SpiTransport]

    B --> F[wound_run]
    F --> F1[compute auto accel/cruise/decel if needed]
    F --> F2[coherence check: profile_turns vs expected_turns]
    F2 -->|error > 5%| F3[logger.warning only]
    F3 --> G[enqueue WoundMove]

    C --> H[_make_wound_streamer]
    H --> H1[from_axis_ids target_hz + segment_duration_s]

    D --> I[_initialize_streamer_state]
    I --> I1[_safe_buffer_time_s(requested, max_hz)]

    C --> J[_execute_wound_move]
    J -->|endstop| J1[invalidate spindle+traverse positions]
    J -->|stop requested| J2[invalidate spindle+traverse positions]
    J -->|success| J3[invalidate spindle+traverse positions]

    A --> K[AppRpcHandler.config]
    K --> K1[dataclass/object safe serialization]
```

---

Validation effectuée pendant la passe V6: `tests/test_motion_v3.py` -> `18 passed`.
