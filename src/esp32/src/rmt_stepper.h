...
public:
    ...
    // Allow host-uploaded ramps to be applied atomically and started.
    // The provided segments array is copied into the internal buffer.
    void set_segments_and_move(const RmtRampSeg* segs, size_t n, int32_t target);
...
public:
    ...
    // Apply host-uploaded ramp segments and start axis motion.
    void apply_segments_and_move(uint8_t axis, const RmtRampSeg* segs, size_t n, int32_t target);
